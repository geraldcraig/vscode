...
<!DOCTYPE html>
    ...
    <tbody>
        <?php
          for($row=0; $row < count($grads); $row++){
            ...
            // challenge number fo years
            //$numyears = $grads[$row]["end"] - $grads[$row]["start"];
            $numyears = $end_y - $start_y;

            // challenge years since graduation
            $yearssince = date("Y") - $end_y;
  ...
  <?php
      ...
      //print_r($grads);
      //echo "<div id='variable'>{$name} started {$uniname} in the year {$startyear} and finished in {$finishyear}. </div>";
